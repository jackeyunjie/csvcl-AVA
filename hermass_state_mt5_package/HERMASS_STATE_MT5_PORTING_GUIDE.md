...
## 第二章：视角决定基准价 —— 这是最核心的概念

> **⚠ 关键纠正：不是所有场景都用 D1 收盘价。视角决定了用哪个周期的收盘价。**

### 2.1 核心原理

State 系统的关键设计是：**视角的基准周期决定所有 position 计算使用的收盘价**。

```
视角 = 你在哪个时间框架上观察 → 该时间框架的收盘价就是所有周期 position 的基准
...
**trend、base、volatility 始终使用各自周期的指标数据——只有 position 的基准价随视角变化。**
...
| 视角 | 回答的问题 | 更新频率 | 典型用途 |
|------|-----------|----------|----------|
| **D1 视角** | "日线价格在各周期结构中的位置" | 每个交易日 | 每日信号触发、适配度、前向观察 |
| **W1 视角** | "周线自身处于什么趋势和位置" | 每个周末 | 周线趋势判断、周度回测、周报 |
| **H1 视角** | "小时价格在各周期结构中的位置" | 每小时 | 盘中实时监控、日内交易 |
| **MN1 视角** | "月线自身处于什么趋势和位置" | 每月末 | 月度宏观判断、长期配置 |

### 2.3 同一标的、同一天、不同视角可能给出不同的 State
...
周三，某股票：
  ...
  D1 视角 W1 State：D1 close(50) > W1 阻力(49) → position = 突破
  W1 视角 W1 State：W1 close(48) < W1 阻力(49) → position = 未突破
...
**关键：在 MT5 上切换图表周期时，State 的 position 基准价会自动变化。** 这是正确行为——不同图表周期对应不同视角。

### 2.5 与 MT5 多周期指标的关系

以 D1 视角为例（最常用），MT5 中的计算对应关系：
...
如果切换为 W1 视角，只需把 position 列的"D1 bar 的收盘价"改为"W1 bar 的收盘价"。

### 2.6 双视角差异率（实测数据）

我们的 Python 系统同时维护 D1 视角和原生视角（W1 close vs W1 SR），实测差异率：
...
**MT5 移植时，必须明确在哪个视角下计算 State，不要混用。**
...
在 D1 视角下：D1 价格变动最快，同一个 SR 区间内 D1 的 position_bit 触发概率远高于月线。同时 D1 的布林带宽变化也更频繁。

**这不是 bug，是设计意图**——日线的高频 E/F 提供短期入场信号，月线的低频 E/F 提供环境框架验证。两者配合使用。

**注意**：如果切换为 W1 视角，W1 的 E/F 出现频率会比 D1 视角下的 W1 更高（因为 W1 close 通常比 D1 close 更稳定地处于某个位置）。这是正确行为。
...
```
日线图上的 W1 State = D1 close vs W1 SR（D1 视角）
周线图上的 W1 State = W1 close vs W1 SR（W1 视角）
...
**MT5 实现建议**：在指标面板上明确标注当前视角（"D1 视角" / "W1 视角"），避免用户混淆。
...
StateResult CalculateState(
    double perspective_close,     // ★视角基准价（D1视角→d1_close, W1视角→w1_close, H1视角→h1_close）
    ...
    // ---- position_bit：视角基准价 vs 本周期SR ----
    // ⚠ perspective_close 取决于当前视角：
    //    D1 视角 → d1_close, W1 视角 → w1_close, H1 视角 → h1_close
...
else perspective_close = d1_close; // 默认 D1 视角
```
...
MT5 的 State 计算结果必须与 Python 端一致。**验证时必须明确当前视角**：

```text
验证 D1 视角：在 MT5 日线图上运行，对比 Python 的 d1_perspective_state
验证 W1 视角：在 MT5 周线图上运行，对比 Python 的 build_weekly_state_independent 输出
```
...
2. **周期视角不同**——有的系统把各周期原生 position 拼接在一起，而非显式选择同一个视角 Agent
...
```text
Step 1: 锁定参数
  - 以本文档的数值为准（4-bit 编码、显式视角 Agent、各窗口参数）
...
| 编号 | 纠正内容 | 影响 |
|------|----------|------|
| C1 | **position 基准价由视角决定，不是固定用 D1 close** | D1 视角→D1 close；W1 视角→W1 close；H1 视角→H1 close |
| C2 | **不同视角下同一标的同一天可能给出不同 State** | 正确行为，不是 bug |
| C3 | **MT5 切换图表周期时，State 计算自动切换视角** | 需要在指标面板标注当前视角 |
| C4 | **trend/base/volatility 始终用各自周期指标** | 只有 position 的基准价随视角变化 |
| C5 | **混合视角是 MT5 上最常见的错误** | 日线图看 W1 State ≠ 周线图看 W1 State |
